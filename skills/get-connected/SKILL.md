...
[`AGENTS.md`](../../../AGENTS.md)). It walks them through two
...
AIOS — a one-paragraph `tools-and-connections/google-workspace.md` recording
...
- **User token, not bot token.** Bot tokens can't read DMs, can't see
  ...
  guidance file is clear about this — keep the scopes under **User
...
When verification passes (`auth.test` returns `ok: true`, the expected
scopes present), the connection is live. Note it in
`tools-and-connections/slack.md` the same way as Google.
...
> in `tools-and-connections/` so future-you and future-AIOSes know it's there."
...
[`AGENTS.md` → Tracking machines and local setup](../../../AGENTS.md).)
...
- **Tools and references.** Connections grant access to the user's
  data; tools and references are different parts of Layer 2. See
  [`AGENTS.md`](../../../AGENTS.md).
